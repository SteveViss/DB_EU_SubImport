...
getwd()
setwd("/Users/Proprietaire/Desktop/Data_DB-EU/Output")

name_table <- read.csv(file.choose()) # Fichier avec le nom pour chaque table
list_files <- list.files("/Users/Proprietaire/Desktop/Data_DB-EU") # Nom des fichiers dans le dossier cible avec les fichiers extraits du ZIP

for (i in 40:dim(name_table)[1]){
  match_list <- as.list(grep(name_table[i,1], list_files, value=TRUE)) #Fait la liste de tous les fichiers contenant la chaine de caractère en i (name_table) dans la liste "list_files" 
  print(name_table)
  
  for (y in 1:length(match_list)){
    
    name <- paste("output",y,sep="") # Nom de la sortie
    path <- paste("/Users/Proprietaire/Desktop/Data_DB-EU", match_list[y], sep="/") # Chemin des fichiers correspondant à la chaine de caractères
    
    print(paste(name_table[i,1],"- File:",y,"/",length(match_list), sep=" "))
  ...
  name_merge <- paste(substr(name_table[i,1],1,nchar(as.character(name_table[i,1]))-4),"_Final_merging", sep="")
  ...
  export_name = paste(name_merge,"csv",sep=".")
  write.csv(lapply(name_merge,get),file=export_name)
  do.call(rm,list(name_merge))
}
